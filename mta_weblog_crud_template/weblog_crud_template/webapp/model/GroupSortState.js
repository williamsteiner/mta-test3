sap.ui.define([
	...
	return BaseObject.extend("com.owi.webmin.weblog_crud_template.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias com.owi.webmin.weblog_crud_template.model.GroupSortState
		 */
		...
		/**
		 * Groups by KEYID, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "KEYID") {
				...
				this._oViewModel.setProperty("/sortBy", "KEYID");

				aSorters.push(
					new Sorter("KEYID", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "KEYTYPE");
			}
