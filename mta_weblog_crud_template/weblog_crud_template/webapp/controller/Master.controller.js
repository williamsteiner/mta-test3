/*global history */
sap.ui.define([
	"com/owi/webmin/weblog_crud_template/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"sap/m/GroupHeaderListItem",
	"sap/ui/Device",
	"com/owi/webmin/weblog_crud_template/model/formatter",
	"sap/m/MessageBox",
	"com/owi/webmin/weblog_crud_template/model/grouper",
	"com/owi/webmin/weblog_crud_template/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, formatter, MessageBox, grouper,
	GroupSortState) {
	"use strict";

	return BaseController.extend("com.owi.webmin.weblog_crud_template.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("KEYTYPE", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function () {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("com.owi.webmin.weblog_crud_template.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function (oEvent) {
			...
			aFilterItems.forEach(function (oItem) {
				switch (oItem.getKey()) {
				case "Filter1":
					aFilters.push(new Filter("KEYID", FilterOperator.LE, 100));
					break;
				case "Filter2":
					aFilters.push(new Filter("KEYID", FilterOperator.GT, 100));
					break;
				default:
					break;
				}
				...
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							ROWID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("ROWID"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", {
				ROWID: encodeURIComponent(oItem.getBindingContext().getProperty("ROWID"))
			}, bReplace);
		},
